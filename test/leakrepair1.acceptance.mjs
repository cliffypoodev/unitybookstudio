...
// the one-word sentence "Messy." was reported as a name variant of "Missy"
...
// ── LEAKREPAIR-1 ──
const live = 'Zin felt the weight in her chest tighten. She remembered the night in Chapter 4, the way she had stared at the stars and felt small.';
const r1 = repairNarrativeMetaLeaks(live);
check('1. the live ch.12 leak is repaired by phrase removal, sentence intact', r1.text === 'Zin felt the weight in her chest tighten. She remembered the night, the way she had stared at the stars and felt small.' && r1.repaired === 1);
...
check('8. a one-word fragment sentence ("Messy.") is not a name variant (live FP dead)', findNameVariants('Rodge stood there and listened. Chaotic. Messy. Loud.\n\nHome.', canon).length === 0);
check('9. a real repeated drift is still caught', (() => { const f = findNameVariants('Mossy grabbed the wrench. Mossy laughed. "Hand it over," Mossy said.', canon); return f.length === 1 && f[0].variant === 'Mossy' && f[0].canonical === 'Missy'; })());
check('10. a sentence-initial name followed by a verb is still counted', findNameVariants('Mossy grabbed the wrench and Rodge stared. Mossy laughed hard.', canon).length === 1);
