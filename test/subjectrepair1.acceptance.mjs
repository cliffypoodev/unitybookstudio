...
const TEXT = [
  'Ludo climbed out of the wreck first. Was wearing his signature leather jacket, though it was now covered in dust. Looked at the horizon and said nothing.',
  'Ottie took the wrench from Yusra. A strange sense of relief wash over her. The weight in her chest lighten.',
  '“Was it always this loud?” JB asked. “Looked fine to me.”',
  'Had Ludo known, he would have stopped. Stop. Wait.',
  '* * *',
...
check('4. a real inversion ("Had Ludo known…") and one-word imperatives are not targets', !sents.some((s) => s.startsWith('Had Ludo')) && !sents.includes('Stop.') && !sents.includes('Wait.'));
...
check('7. a cast name is a legal subject', verifySubjectRepair(o1, 'Ludo was wearing his signature leather jacket, though it was now covered in dust.', { castNames: CAST }).ok);
...
check('9. an unknown subject is rejected (closed set)', !verifySubjectRepair(o1, 'Idris was wearing his signature leather jacket, though it was now covered in dust.', { castNames: CAST }).ok);
...
const stub = async (userPrompt) => {
  ...
  if (s.startsWith('Looked at the horizon')) return 'Ludo looked at the horizon and said nothing.';
...
check('13. verified repairs are applied in place', healed.text.includes('Ludo climbed out of the wreck first. He was wearing his signature leather jacket') && healed.text.includes('Ludo looked at the horizon and said nothing.') && healed.text.includes('She felt a strange sense of relief wash over her.') && healed.text.includes('They were a beautiful, chaotic mess.'));
...
check('15. dialogue and the rest of the text are byte-identical', healed.text.includes('“Was it always this loud?” JB asked. “Looked fine to me.”') && healed.text.includes('Had Ludo known, he would have stopped. Stop. Wait.') && healed.text.includes('\n\n* * *\n\n'));
...
check('18b. a curly/straight apostrophe difference is not a content change (live: 40+ good repairs were rejected)', verifySubjectRepair('Looked at the wrench in Yusra’s hand.', "Ottie looked at the wrench in Yusra's hand.", { castNames: CAST }).ok);
check('18c. the applied text keeps the book’s typography', verifySubjectRepair('Looked at the wrench in Yusra’s hand.', "Ottie looked at the wrench in Yusra's hand.", { castNames: CAST }).applied === 'Ottie looked at the wrench in Yusra’s hand.');
...
{
  const cleanText = 'Ottie walked into the room and sat down at the table quietly.';
  const lines = [];
